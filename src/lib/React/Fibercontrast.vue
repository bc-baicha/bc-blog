<template>
  ...
  <PageContent>
    ...
    <h3>React Fiber简述：</h3>
    <h4>同步更新过程的局限：递归调用--渲染界面</h4>
    <p>
      当React调用和更新组件时，会调用生命周期函数，计算和比对虚拟DOM，最后更新DOM树，这整个过程都是同步的，也就是说一个更新或者加载过程，开始了中间就不能中断，如果组件少的的时候，一个组价需要一毫秒，但是当组件数量庞大时，浏览器主线程一直在执行更新或者加载操作，不会处理其它的操作，如果这时一直执行输入操作，不会得到反应，等到React渲染完成了，才会响应输入的操作，然后输入的值就会出现在输入框中，用户体验感很差，就会造成所谓的界面卡顿的现象。
    </p>
    <h4>Fiber就是为了改变这个状况：</h4>
    <p>
      维护每一个分片的数据结构，就是Fiber。<br />
      Fiber的解决方法是分片，把一个耗时很长的任务分成很多小片，每一个小片的时间很短，但是每个小片执行完之后，都会把控制权交换给React让它去协调其它的任务模块，看看有没有优先级高的紧急任务，如果有就去执行紧急任务，如果没有就继续更新操作，这样唯一的主线程就不会一直被占用。
    </p>
    <h4>一个Fiber的更新过程分为两段：</h4>
    <p>
      第一个阶段是协调阶段，找出需要更新哪些DOM元素，这个阶段是可以被打断的。<br />
      第二阶段就是更新阶段，这个阶段就是一下把更新操作做完，不能被打断。
    </p>
  </PageContent>
</template>
